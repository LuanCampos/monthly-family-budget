...
/**
 * Converts HSL string (e.g., "30 6% 9%") to hex color (e.g., "#181614")
 */
const hslToHex = (hslString: string): string => {
  const parts = hslString.match(/[\d.]+/g);
  if (!parts || parts.length < 3) return '#181614'; // fallback
...
/**
 * Updates the theme-color meta tag for browser/PWA status bar
 */
const updateThemeColorMeta = (theme: ThemeKey): void => {
  const bgHsl = themeVariables[theme]['--background'];
...
const applyTheme = (theme: ThemeKey) => {
  ...
  // Update browser/PWA theme-color meta tag
